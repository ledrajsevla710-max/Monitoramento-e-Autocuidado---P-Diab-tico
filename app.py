    ...
    if not st.session_state.authenticated:
        ...
        with tab_cadastro:
            ...
            if st.button("FINALIZAR CADASTRO"):
                if novo_nome and novo_email and nova_senha and nova_cidade:
                    try:
                        df_users_at = conn.read(worksheet="usuarios", ttl=0)
                        df_users_at.columns = [str(c).strip().lower() for c in df_users_at.columns]

                        novo_user = pd.DataFrame([{
                            "nome": str(novo_nome).strip(),
                            "email": str(novo_email).strip().lower(),
                            "senha": str(nova_senha).strip(),
                            "cidade": str(nova_cidade).strip(),
                            "uf": str(uf_reg).strip()
                        }])

                        df_final = pd.concat([df_users_at, novo_user], ignore_index=True)
...
if auth_system():
    ...
    .flashcard { padding: 20px; border-radius: 15px; background-color: #e3f2fd; border: 1px solid #90caf9; text-align: center; margin-bottom: 10px; font-weight: bold; color: #0d47a1; }
    ...
    if "lembrete_ok" not in st.session_state:
        st.session_state.lembrete_ok = False
    ...
    if not st.session_state.lembrete_ok:
        ...
        if st.button("✔ Já olhei meus pés hoje"):
            st.session_state.lembrete_ok = True
            st.success("Ótimo! O autocuidado diário é essencial 👏")
            st.rerun()
    ...
    if st.session_state.etapa == 1:
        ...
        uf = st.selectbox("UF", lista_uf, index=17)
    ...
    # --- ETAPA 2: AVALIAÇÃO ---
        ...
        with col2:
            ...
            st.markdown('<div class="flashcard">👀 Observe seus pés todos os dias</div>', unsafe_allow_html=True)
            st.markdown('<div class="flashcard">🧴 Hidrate (não entre os dedos)</div>', unsafe_allow_html=True)
            st.markdown('<div class="flashcard">👟 Use calçados adequados</div>', unsafe_allow_html=True)
